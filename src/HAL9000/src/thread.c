...
typedef struct _THREAD_SYSTEM_DATA
{
	...
	//Project_1
	LOCK ReadyThreadsPriorityListLocks[32];

	_Guarded_by_(ReadyThreadsPriorityListLocks)
		LIST_ENTRY ReadyThreadsPriorityLists[32];

	APIC_ID MinPriorityCPU;

	THREAD_PRIORITY MinimumRunningPriority;
...
)
{
	...
	for (BYTE i = 0; i < 32; i++)
	{
		InitializeListHead(&m_threadSystemData.ReadyThreadsPriorityLists[i]);
		LockInit(&m_threadSystemData.ReadyThreadsPriorityListLocks[i]);
	}

	m_threadSystemData.MinPriorityCPU = 0;
	m_threadSystemData.MinimumRunningPriority = ThreadPriorityReserved;
}
...
)
{
	...
	if (pThread != pCpu->ThreadData.IdleThread)
	{
		...
		//Project_1
		THREAD_PRIORITY ComputedPriority = ThreadGetPriority(pThread);
		LockAcquire(&m_threadSystemData.ReadyThreadsPriorityListLocks[ComputedPriority], &dummyState);
		InsertTailList(&m_threadSystemData.ReadyThreadsPriorityLists[ComputedPriority], &(pThread->ReadyThreadsPriorityListEntry));
		LockRelease(&m_threadSystemData.ReadyThreadsPriorityListLocks[ComputedPriority], dummyState);
...
//Project_1
THREAD_PRIORITY
ThreadGetPriorityRecursive(
	IN PTHREAD Thread,
	BYTE MaxRecursionDepth
)
{
	...
	THREAD_PRIORITY MaxPriority = pThread->StaticPriority;
	if (ListSize(&pThread->HeldMutexesList) == 0)
	{
		return MaxPriority;
	}
	
	LIST_ITERATOR MutexIterator;
	ListIteratorInit(&pThread->HeldMutexesList, &MutexIterator);
	PLIST_ENTRY pEntry;
	//LOGP("RECURSIVE depth %d\n", MaxRecursionDepth);
	while ((pEntry = ListIteratorNext(&MutexIterator)) != NULL)
	{
		PMUTEX pMutex = CONTAINING_RECORD(pEntry, MUTEX, HeldMutexesListEntry);
		LIST_ITERATOR ThreadIterator;
		ListIteratorInit(&pMutex->WaitingList, &ThreadIterator);
		PLIST_ENTRY pEntryReadyThreads;
		while ((pEntryReadyThreads = ListIteratorNext(&ThreadIterator)) != NULL)
		{
			PTHREAD pThreadDonor = CONTAINING_RECORD(pEntryReadyThreads, THREAD, ReadyList);
			THREAD_PRIORITY DonatedPriority = ThreadGetPriorityRecursive(pThreadDonor, MaxRecursionDepth - 1);
			if (DonatedPriority > MaxPriority)
			{
				MaxPriority = DonatedPriority;
			}
		}
	}
...
)
{
	...
	//Project_1
	THREAD_PRIORITY ComputedPriority = ThreadGetPriority(Thread);
	LockAcquire(&m_threadSystemData.ReadyThreadsPriorityListLocks[ComputedPriority], &dummyState);
	InsertTailList(&m_threadSystemData.ReadyThreadsPriorityLists[ComputedPriority], &(Thread->ReadyThreadsPriorityListEntry));
	LockRelease(&m_threadSystemData.ReadyThreadsPriorityListLocks[ComputedPriority], dummyState);
...
//returns max static_priority and donated priority
THREAD_PRIORITY
ThreadGetPriority(
	IN_OPT  PTHREAD             Thread
)
{
	...
	return ThreadGetPriorityRecursive(pThread, MAX_RECURSION_DEPTH);
}
...
)
{
	...
	__finally
	{
		...
		//Project_1
		LockInit(&pThread->HeldMutexesLock);
		InitializeListHead(&pThread->HeldMutexesList);
		pThread->StaticPriority = pThread->Priority;
...
)
{
	...
	if (pNextThread != pCurrentThread)
	{
		...
		if (pNextThread->Priority < m_threadSystemData.MinimumRunningPriority)
		{
			m_threadSystemData.MinimumRunningPriority = pNextThread->Priority;
			m_threadSystemData.MinPriorityCPU = pCpu->ApicId;
			
			SMP_DESTINATION dst;
			dst.Cpu.ApicId = m_threadSystemData.MinPriorityCPU;
			SmpSendGenericIpiEx(_ThreadScheduleIpi, NULL, NULL, NULL, FALSE, SmpIpiSendToCpu, dst);
		}
...
)
{
	...
	char i = 31;
	while (i >= 0 && IsListEmpty(&m_threadSystemData.ReadyThreadsPriorityLists[i--]));
	i++;
	pEntry = RemoveHeadList(&m_threadSystemData.ReadyThreadsPriorityLists[i]);
	if (pEntry == &m_threadSystemData.ReadyThreadsPriorityLists[i])
	{
	...
	{
		pNextThread = CONTAINING_RECORD(pEntry, THREAD, ReadyThreadsPriorityListEntry);
